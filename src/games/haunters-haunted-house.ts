		...
		for (const id in this.players) {
			...
			for (const ghost of this.ghosts) {
				...
				if (((xDifference === 0 && yDifference === 0) || (ghost.hauntNextTurn && xDifference <= 1 && yDifference <= 1)) &&
					ghost.name !== "Mimikyu" && !player.eliminated) {
...
const commands: GameCommandDefinitions<HauntersHauntedHouse> = {
	...
	createdoor: {
		command(target, room, user, cmd) {
			...
			for (let i = 0; i < this.boardSize; i++) {
				for (let j = 0; j < this.boardSize; j++) {
					if ((this.board[i][j] as Door).number && num === (this.board[i][j] as Door).number) {
						this.say("A door with the number " + num + " already exists.");
						return false;
					}
				}
			}
